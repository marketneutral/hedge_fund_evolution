...
subtitle: "Guest Lecture for MIT 18.642<br>*Topics in Mathematics with Applications in Finance*"
...
## Condorcet Jury Theorem (1785) {.smaller}
::: {.incremental}
...
- Generalizes to multi-class classification (not just binary)
...
- Make quadratic and interaction features
...
- Examples: **Numerai**, **Yiedl**, **CrunchDAO**, **CrowdCent**
:::
...
- Community data scientists can run models locally and submit via API (keep control of code and IP)
...
- Founded 2023, Italy based, fully **DAO-based hedge fund**  
...
## Yiedl – Community & Competitions {.smaller}
::: columns
::: column
...
- Founded 2021, Paris/Dubai/NYC-based, **decentralized data science collective**  
- Hosts **ML competitions** to solve general business and hard science problems
- Some challenges in finance
    - DataCrunch Equity Market Neutral
    - Venture Capital Portfolio Prediction
    - "Mid+One" high-frequency trading challenge
    - ADIA Lab Structural Break Challenge

![](https://cdn.prod.website-files.com/685017b47dcdacbd34a6867f/68502c383d9a29f1dad265b9_logo.svg){.absolute top=0 right=0 width="15%"}
---

## CrunchDAO – Community & Competitions {.smaller}
::: columns
::: column
- **5,000+ contributors**, including ~600 PhDs  
- Participants submit ML models on anonymized datasets  
- Scored on withheld data → leaderboards & rankings  
- **Rewards:**  
  - Prize pools currently paid in USDC  
- Community predictions are **aggregated into consensus signals**
- Meta model used by institutional clients
:::
::: column
![](https://blog.crunchdao.com/wp-content/uploads/2025/06/ad_4nxck8-_qw436szjkmb-3gtasvp_e9-feyngu5euf0_po75jwphkknj8s0sltikhdpn91t51ojvjryh83vm63bcsu6sozkoxd2pqpoplf4mhih7yqyhjbgvayd6dxpksudhqp57jkvw.png){width="90%"}
:::
:::

## CrunchDAO – Data & Rules {.smaller}

- **Company/Coordinator:** curates anonymized tabular datasets
- Features are obfuscated; time-sliced equity & financial data  
- **Rules:**  
  - No external data; models must run only on provided features  
  - Code submissions (not just predictions) in controlled environments (Python/R)  
  - Compute quotas (e.g., GPU-hour limits) to ensure fairness  
- **Evaluation:**  
  - Out-of-sample scoring (e.g. Spearman correlation)  
  - Transparent leaderboards, confidential models  
- Reproducibility supported with open docs & starter notebooks


## CrunchDAO – Role of Crypto

- Currently, Crypto enables:  
  - Global, micro-payouts at scale  
  - Transparent, trustless settlement  
- In the future, **CRUNCH token:** native incentive + governance unit  
- **Governance DAO:** contributors shape rules, approve contests, manage treasury  
...
- **CrunchDAO:** decentralized data science collective solving diverse problems; acts as intermediary
...
![](images/hmcollab.png.png){.absolute bottom=0 right=0 width="65%"}
...
- Compare performance:
  ...
  - *Machine only*: an ensemble of ML algorithms (random forests, boosting, LSTM and) trained on the features to predict year-ahead stock prices, *excluding any analyst forecast inputs*
...
## Attempt upgrades for others {.smaller}

- For each human pick that isn’t top-decile, try to replace it with the single best available top-decile stock from the same style group, subject to liquidity limits.
...
for j in holdings sorted by descending w_h[j]:
    if decile[j] in 1..9:
        group := g[j]   # DGTW group of stock j
        # find candidates in same group, top decile prediction, not already used
        ...
        if C ≠ ∅:
            ...
            used.add(k)
```
## Replace remaining bottom-decile names {.smaller}
- Any stock the AI thinks is terrible gets dropped entirely and replaced with its group index (e.g., "large cap value ETF"), so the portfolio stays in-style but doesn’t carry dead weight. Then we rebalance the whole thing to make sure weights sum to 1.
```{python}
#| eval: false
#| echo: true
#| code-line-numbers: "1-2|3|4|5-6|8-9"
# Replace remaining bottom-decile names with the group style index
for j in holdings:
